...
public class LoyaltyEntryPointFactory implements EntryPointFactory {
	...
	private int createWorkbench() {
		...
		final E4Application e4App = new E4Application();
		final E4Workbench workbench = e4App.createE4Workbench(getApplicationContext(config), display);

		if (config.isDefaultPush()) {
			final ServerPushSession session = new ServerPushSession();
			session.start();
		}
		...
		if (workbench != null) {
			...
			final IEclipseContext serviceContext = workbenchContext.getParent();

			// Create and run the UI (if any)

			final LoginContext loginContext = (LoginContext) workbenchContext.get("secureContext");

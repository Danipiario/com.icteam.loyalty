...
public final class Loader {
	...
	/**
	 * Returns the ClassLoader of current thread if possible, or falls back to
	 * the system ClassLoader if none is available.
	 *
	 * @return the TCCL.
	...
	/**
	 * This method will search for {@code resource} in different places. The
	 * search order is as follows:
	 *
	 * <ol>
	 *
	 * <li>Search for {@code resource} using the thread context class loader
	 * under Java2. If that fails, search for {@code resource} using the class
	 * loader that loaded this class ({@code Loader}). Under JDK 1.1, only the
	 * the class loader that loaded this class ({@code Loader}) is used.</li>
	 * <li>Try one last time with
	 * {@code ClassLoader.getSystemResource(resource)}, that is is using the
	 * system class loader in JDK 1.2 and virtual machine's built-in class
	 * loader in JDK 1.1.</li>
	 * </ol>
	 *
	 * @param resource
	...
	public static URL getResource(final String resource, final ClassLoader defaultLoader) {
		...
		String r = System.getProperty(resource);
		if (r != null) {
			try {
	 ...
	 * that a {@code null} ClassLoader is interpreted as the system ClassLoader
	 * as per convention.
	 *
	 * @param loader1
	 *            the ClassLoader to check for childhood.
	 * @param loader2
	 *            the ClassLoader to check for parenthood.
	...
	private static boolean isChild(final ClassLoader loader1, final ClassLoader loader2) {
		if (loader1 != null && loader2 != null) {
			...
			// once parent is null, we're at the system CL, which would indicate
			// they have separate ancestry
			return parent != null;
		}
		return loader1 != null;
	}
	...
	/**
	 * Load a Class by name. Note that unlike
	 * {@link ClassLoader#loadClass(String) ClassLoader.loadClass}, this method
	 * will initialize the class as well if it hasn't been already. This is
	 * equivalent to the calling the
	 * {@link ClassLoader#loadClass(String, boolean) protected version} with the
	 * second parameter equal to {@code true}.
	 *
	 * @param className
	 *            The class name.
	 * @return The Class.
	...
	/**
	 * Load a Class in the {@code java.*} namespace by name. Useful for peculiar
	 * scenarios typically involving Google App Engine.
	 *
	 * @param className
	 *            The class name.
	 * @return The Class.
	 ...
	 * @throws ClassNotFoundException
	 *             if the class isn't available to the usual ClassLoaders
	 * @throws IllegalAccessException
	 *             if the class can't be instantiated through a public
	 *             constructor
	 * @throws InstantiationException
	 *             if there was an exception whilst instantiating the class
	 * @throws NoSuchMethodException
	 *             if there isn't a no-args constructor on the class
	 * @throws InvocationTargetException
	 *             if there was an exception whilst constructing the class
	 */
	 ...
	 *             if the constructed object isn't type compatible with
	 *             {@code T}
	 */
	 ...
	 * @param className
	 *            The class name.
	 * @return {@code true} if the class could be found or {@code false}
	 *         otherwise.
	 */

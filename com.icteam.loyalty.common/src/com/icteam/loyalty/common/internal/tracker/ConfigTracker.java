...
public class ConfigTracker extends BundleTracker<Bundle> {
	...
	public ConfigTracker(BundleContext context) {
		...
		final String installDir = System.getProperty("felix.fileinstall.dir");
		if (installDir != null) {
			configuration = new File(installDir.split(",")[0]);

			if (!configuration.exists()) {
				configuration.mkdirs();
			}
		} else {
			logger.info("ConfigTracker not enabled. Please set felix.fileinstall.dir property");
		}
	}
	...
	private boolean hasConfiguration(Bundle bundle) {
		return bundle.getEntry(CONFIGURATION) != null && configuration != null;
	}
}
